...
num_pyr = 400 # Pyramidal neurons
num_inh = 100 # Inhibitory neurons
...
gampap = 1.9 # AMPA on pyramidal neurons [nS]
ggabap = 25 # GABA on pyramidal neurons [nS]
gnmdap = 0.6 # NMDA on pyramidal neurons [nS]
...
gampai = 3 # AMPA on inhibitory neurons [nS]
ggabai = 40 # GABA on inhibitory neurons [nS]
gnmdai = 1 # NMDA on inhibitory neurons [nS]
...
#    crate[i+1] = np.sum(vc[i,:]==Vpeak)*SR
...
bin_size = 1 # [ms]
i_rate_sum = pd.rolling_sum(irate/SR,bin_size/dt)*(1e3/bin_size)/num_inh
p_rate_sum = pd.rolling_sum(prate/SR,bin_size/dt)*(1e3/bin_size)/num_pyr
plt.figure()
plt.plot(t[14000:16000],i_rate_sum[14000:16000])
plt.plot(t[14000:16000],p_rate_sum[14000:16000],'r')
plt.xlabel('time [ms]')
plt.ylabel('network rate [spikes/s]')
i_rate = pd.rolling_window(irate/num_inh,window=int(bin_size/dt),win_type='hamming')
p_rate = pd.rolling_window(prate/num_pyr,window=int(bin_size/dt),win_type='hamming')
plt.figure()
