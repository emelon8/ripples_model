...
FILE *volt1, *volt501, *volt1001, *volt1501, *volt2001, *volt2501, *volt3001, *volt3501, *volt4001, *volt4501, *rateei, *rateneurons, *nM1, *nM4501;
...
float gM, EM, MVhalf, Mk, MVmax, Msigma, MCamp, MCbase, gMe, gMi;
float DCe, DCi, DC;
...
void run_sim(long idum)
{
	...
	for (icycle = 0; icycle < sub*tsimul/**&&exact[icycle/sub]<NE/3**/; icycle++) {
		...
		tauarp = tarpE; zdec = zdecaye; tdec = taumE; dtm = dtme; freq = nuE; nuext = nuexte; gM = gMe_tmp; DC = DCe;
		...
		for (ineur = 0; ineur < nneur; ineur++) {
			if (ineur == NE) { //sets the parameters to the parameters for inhibitory neurons from here on (for this time step)
				...
				tauarp = tarpI; zdec = zdecayi; tdec = taumI; dtm = dtmi; freq = nuI; nuext = nuexti; gM = gMi_tmp; DC = DCi;
			...
			fspk = hrece[ineur][tminx]; // counts excitatory inputs to the cell
			...
			IAMPA = coefAMPA * sAMPA[ineur] * (VE - hi[ineur]); // coefAMPA = 0.1875 nS; max(sAMPA) = 0.2672; so nS * mV = pA. At -55 mV, = 2.7555 pA, which produces a voltage change of 0.1102 mV.
			...
			hik1 = dtm*(IEXT + IAMPA + INMDA + IGABA + IM + DC - hi[ineur]);
			...
			hik2 = dtm*(IEXT + IAMPArk + INMDArk + IGABArk + IMrk + DC - hirk);
			sEXT[ineur] += sEXTk2;
			hi[ineur] += hik2;
			nM[ineur] += nMk2;
...
int main()
{
	...
	globaltime = 0;
	tsimul = tprestim;
	taumE = 20.0; //20.0 originally, 40.0 with 12.5 nS M-current; used the same values as in Wang 1999, i.e.gL = 0.025 microS, Cm = 0.5nF for pyr cells,
	taumI = 10.0; //10.0 originally, 27.0 with 12.5 nS M-current; gL = 0.02 microS Cm = 0.2nF for interneurons
	tauGABAedecay = 5.;
	tauGABAidecay = 5.;
	tauGABArise = 0.5;
	...
	gMe = 0;//nS 12.5; // taken from Ermentrout and Terman, 2010, p. 85 (which took it from Destexhe and Pare, 1999?).
	gMi = 0;
	EM = -7.; // all these taken from Izhikevich, 2007, p. 45, 47 (which took it from Robbins et al., 1992).
	MVhalf = -44. + 70.;
	Mk = 8.;
	MVmax = -50. + 70.;
	Msigma = 25.;
	...
	taumE = 0.5e-9 * (1 / ((25. - gMe)*1e-9)) * 1e3; // * 1e3 to get it into ms
	for (DCi = 0; DCi < 11 * 20; DCi = DCi + 20) { //20pA/mV in interneurons (25 for pyramidal neurons)
		taumI = 0.2e-9 * (1 / ((20. - gMi)*1e-9)) * 1e3; // * 1e3 to get it into ms

		char bufferrateei[32], bufferrateneurons[32]; // The filename buffers.
		// Put "file" then k then ".txt" in to filename.
		sprintf(bufferrateei, "rateei_%dDCe%dDCi.txt", (int)DCe, (int)DCi);
		sprintf(bufferrateneurons, "rateneurons_%dDCe%dDCi.txt", (int)DCe, (int)DCi);
		//fopen_s(&volt1, "10000ms_voltage_1-500_voltages.txt", "a");
		...
		//fopen_s(&volt4501, "10000ms_voltage_4501-5000_voltages.txt", "a");
		rateei=fopen(bufferrateei, "a");
		rateneurons=fopen(bufferrateneurons, "a");
		//fopen_s(&nM1, "10000ms_nM_1-500.txt", "a");
		//fopen_s(&nM4501, "10000ms_nM_4501-5000.txt", "a");
		run_sim(idum);
		//fclose(volt1); fclose(volt4501); //fclose(volt501); fclose(volt1001); fclose(volt1501); fclose(volt2001); fclose(volt2501); fclose(volt3001); fclose(volt3501); fclose(volt4001);
		fclose(rateei);
		fclose(rateneurons);
		//fclose(nM1); fclose(nM4501);
	}
