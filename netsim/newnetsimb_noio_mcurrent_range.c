...
void run_sim(long idum)
{
	...
	for (icycle = 0; icycle < sub*tsimul/**&&exact[icycle/sub]<NE/3**/; icycle++) {
		...
		tauarp = tarpE; zdec = zdecaye; tdec = taumE; dtm = dtme; freq = nuE; nuext = nuexte; gM = gMe_tmp;
		...
		for (ineur = 0; ineur < nneur; ineur++) {
			if (ineur == NE) { //sets the parameters to the parameters for inhibitory neurons from here on (for this time step)
				...
				tauarp = tarpI; zdec = zdecayi; tdec = taumI; dtm = dtmi; freq = nuI; nuext = nuexti; gM = gMi_tmp;
...
int main()
{
	...
	globaltime = 0;
	tsimul = tprestim;
	taumE = 40.0; //20.0 originally, 40.0 with 12.5 nS M-current; used the same values as in Wang 1999, i.e.gL = 0.025 microS, Cm = 0.5nF for pyr cells,
	...
	gMe = 12.5;//nS 12.5; // taken from Ermentrout and Terman, 2010, p. 85 (which took it from Destexhe and Pare, 1999?).
	gMi = 0;
	...
	for (gMe = 0; gMe < 19; gMe = gMe + 2) {
		taumE = 0.5e-9 * (1 / ((25. - gMe)*1e-9)) * 1e3; // * 1e3 to get it into ms
		for (gMi = 0; gMi < 19; gMi = gMi + 2) {
			taumI = 0.2e-9 * (1 / ((20. - gMi)*1e-9)) * 1e3; // * 1e3 to get it into ms
			for (onoff = 1; onoff > -1; onoff--) {

				char bufferrateei[32], bufferrateneurons[32]; // The filename buffers.
				// Put "file" then k then ".txt" in to filename.
				sprintf_s(bufferrateei, sizeof(char) * 32, "rateei_%dgMe%dgMi%d.txt", (int)gMe, (int)gMi, onoff);
				sprintf_s(bufferrateneurons, sizeof(char) * 32, "rateneurons_%dgMe%dgMi%d.txt", (int)gMe, (int)gMi, onoff);
				...
				fopen_s(&rateei, bufferrateei, "a");
				fopen_s(&rateneurons, bufferrateneurons, "a");
