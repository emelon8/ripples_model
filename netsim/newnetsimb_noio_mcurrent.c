...
#define ivisual 0	
/****************** Neurons ********************/
#define taumE 40.0 //20.0 originally, 40.0 with 12.5 nS M-current; used the same values as in Wang 1999, i.e.gL = 0.025 microS, Cm = 0.5nF for pyr cells,
#define taumI 10.0 //10.0 originally, 27.0 with 12.5 nS M-current; gL = 0.02 microS Cm = 0.2nF for interneurons
...
void run_sim(long idum)
{
	...
	for (icycle = 0; icycle < sub*tsimul/**&&exact[icycle/sub]<NE/3**/; icycle++) {
		...
		tauarp = tarpE; zdec = zdecaye; tdec = taumE; dtm = dtme; freq = nuE; nuext = nuexte; gM = gMe;
		...
		for (ineur = 0; ineur < nneur; ineur++) {
			if (ineur == NE) { //sets the parameters to the parameters for inhibitory neurons from here on (for this time step)
				...
				tauarp = tarpI; zdec = zdecayi; tdec = taumI; dtm = dtmi; freq = nuI; nuext = nuexti; gM = gMi;
			...
			hik1 = dtm*(IEXT + IAMPA + INMDA + IGABA + IM - hi[ineur]); // WHY IS THIS CURRENTS + A VOLTAGE??? I UNDERSTAND THAT THE LEAK REVERSAL IS 0, BUT gL*(0-hi[ineur]) STILL HAS THE gL. WHERE IS THAT?
...
int main()
{
	...
	gMe = 12.5;// 12.5; // taken from Ermentrout and Terman, 2010, p. 85 (which took it from Destexhe and Pare, 1999?).
	gMi = 0;
	...
	fopen_s(&rateei, "10000ms_rateei_12.5mcurrentE40taumE.txt", "a");
	fopen_s(&rateneurons, "10000ms_rateneurons_12.5mcurrentE40taumE.txt", "a");
