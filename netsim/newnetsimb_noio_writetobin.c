...
FILE *volt1, *volt501, *volt1001, *volt1501, *volt2001, *volt2501, *volt3001, *volt3501, *volt4001, *volt4501, *rateei, *rateneurons;

float this_neuron_voltage1[500][200000]; // 200000 = tprestim / dt

//struct voltage {
//	int neuron_number;
//	float neuron_time, neuron_voltage;
//};
...
void run_sim(long idum)
{
	...
	for (int ilk = 0; ilk < 500; ilk++) {
		for (int kilk = 0; kilk < 200000; kilk++) {
			this_neuron_voltage1[ilk][kilk] = 0.;
		}
	}
	...
	for (icycle = 0; icycle < sub*tsimul/**&&exact[icycle/sub]<NE/3**/; icycle++) {
		...
		for (ineur = 0; ineur < nneur; ineur++) {
			...
			//struct voltage write_voltage;
			//write_voltage.neuron_number = ineur;
			//write_voltage.neuron_time = (float)icycle / (float)sub;
			//write_voltage.neuron_voltage = hi[ineur];
			if (ineur<500) this_neuron_voltage1[ineur][icycle]=hi[ineur];
			if (ineur >= 0 && ineur < 500) {
				fwrite(&this_neuron_voltage1, sizeof(float), 1, volt1); //(float)(icycle) / (float)(sub)+globaltime
				fprintf(volt1001, "%f\n", this_neuron_voltage1); //(float)(icycle) / (float)(sub)+globaltime
			}
			//if (ineur >= 500 && ineur < 1000) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt501); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 1000 && ineur < 1500) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt1001); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 1500 && ineur < 2000) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt1501); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 2000 && ineur < 2500) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt2001); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 2500 && ineur < 3000) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt2501); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 3000 && ineur < 3500) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt3001); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 3500 && ineur < 4000) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt3501); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 4000 && ineur < 4500) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt4001); //(float)(icycle) / (float)(sub)+globaltime
			//}
			//if (ineur >= 4500 && ineur < 5000) {
			//	fwrite(&this_neuron_voltage, sizeof(float), 1, volt4501); //(float)(icycle) / (float)(sub)+globaltime
			//}
		}
		tminx = icycle%CYCSIZE;
		...
		if (icycle%sub == sub - 1) {
			...
			fprintf(rateei, "%d,%d,%d\n", icycle / sub, exact[icycle / sub], inact[icycle / sub]);
		}
	}
	...
	for (ineur = 0; ineur < NE + NI; ineur++) fprintf(rateneurons, "%d,%d\n", ineur, spkno[ineur]);
}
...
int main()
{
	...
	fopen_s(&volt1, "10000ms_voltage_1-500_voltages.bin", "a");
	fopen_s(&volt501, "10000ms_voltage_501-1000_voltages.bin", "a");
	fopen_s(&volt1001, "10000ms_voltage_1001-1500_voltages.txt", "a");
	fopen_s(&volt1501, "10000ms_voltage_1501-2000_voltages.bin", "a");
	fopen_s(&volt2001, "10000ms_voltage_2001-2500_voltages.bin", "a");
	fopen_s(&volt2501, "10000ms_voltage_2501-3000_voltages.bin", "a");
	fopen_s(&volt3001, "10000ms_voltage_3001-3500_voltages.bin", "a");
	fopen_s(&volt3501, "10000ms_voltage_3501-4000_voltages.bin", "a");
	fopen_s(&volt4001, "10000ms_voltage_4001-4500_voltages.bin", "a");
	fopen_s(&volt4501, "10000ms_voltage_4501-5000_voltages.bin", "a");
	fopen_s(&rateei, "10000ms_rateei.txt", "a");
	fopen_s(&rateneurons, "10000ms_rateneurons.txt", "a");
	run_sim(idum);
	fclose(volt1); fclose(volt501); fclose(volt1001); fclose(volt1501); fclose(volt2001); fclose(volt2501); fclose(volt3001); fclose(volt3501); fclose(volt4001); fclose(volt4501);
	fclose(rateei);
	fclose(rateneurons);
