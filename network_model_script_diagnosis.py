...
# my Python functions
import lif_neuron_diagnosis as lif
...
sap = np.zeros((len(t),num_pyr))
sgp = np.zeros((len(t),num_pyr))
snp = np.zeros((len(t),num_pyr))
Rp = np.zeros((len(t),num_pyr))
...
sai = np.zeros((len(t),num_inh))
sgi = np.zeros((len(t),num_inh))
sni = np.zeros((len(t),num_inh))
Ri = np.zeros((len(t),num_inh))
...
#sechat = se_poisson(t,dt,Iechatrate,num_chat,sampakernelp)
...
print('you made it to the for loop at %s!' % time())
...
#    sdi[i+1,vi[int(np.rint(i-taulampa/dt)),:]>0] = sdi[i,vi[int(np.rint(i-taulampa/dt)),:]>0] + 1
    ...
    vp[i+1,:], arp_counterp[i+1,:], Ip[i+1,:], sap[i+1,:], sgp[i+1,:], snp[i+1,:], Rp[i+1,:] = lif.LIF_neuron(dt,vp[i,:],taump,gLp,Vrestp,Vrp,Vtp,arpp,arp_counterp[i,:],sepyr[i,:],con_matrix[:,:num_pyr],num_pyr,num_inh,num_chat,sampap[i,:],sgabap[i,:],snmdap[i,:],gampap,ggabap,gnmdap,gampape,vampap,vgabap,vnmdap,DCp) # pyramidal neurons
    vi[i+1,:], arp_counteri[i+1,:], Ii[i+1,:], sai[i+1,:], sgi[i+1,:], sni[i+1,:], Ri[i+1,:] = lif.LIF_neuron(dt,vi[i,:],taumi,gLi,Vresti,Vri,Vti,arpi,arp_counteri[i,:],seinh[i,:],con_matrix[:,num_pyr:num_pyr+num_inh],num_pyr,num_inh,num_chat,sampai[i,:],sgabai[i,:],snmdai[i,:],gampai,ggabai,gnmdai,gampaie,vampai,vgabai,vnmdai,DCi) # inhibitory neurons
...
#    crate[i+1] = np.sum(vc[i,:]==Vpeak)*SR/num_chat
    ...
    if i%(1000/dt) == 0:
        print('you made it to the next second at %s!' % time())
...
np.savez_compressed("saved_variables_diagnosis_all",DCi=DCi,DCp=DCp,Ieinhrate=Ieinhrate,Iepyrrate=Iepyrrate,Ii=Ii,Ip=Ip,Ri=Ri,Rp=Rp,SR=SR,Vpeak=Vpeak,Vresti=Vresti,
         ...
         pyr_chat_pr=pyr_chat_pr,pyr_inh_pr=pyr_inh_pr,pyr_pyr_pr=pyr_pyr_pr,sai=sai,sampai=sampai,sampakerneli=sampakerneli,sampakernelp=sampakernelp,
         sampap=sampap,sampapeaki=sampapeaki,sampapeakp=sampapeakp,sap=sap,sdampai=sdampai,sdampap=sdampap,sdgabai=sdgabai,sdgabap=sdgabap,
         sdnmdai=sdnmdai,sdnmdap=sdnmdap,seinh=seinh,sepyr=sepyr,sgabai=sgabai,sgabakerneli=sgabakerneli,
         sgabakernelp=sgabakernelp,sgabap=sgabap,sgabapeaki=sgabapeaki,sgabapeakp=sgabapeakp,sgi=sgi,sgp=sgp,simulation_time=simulation_time,
         sni=sni,snmdai=snmdai,snmdakerneli=snmdakerneli,snmdakernelp=snmdakernelp,snmdap=snmdap,snmdapeaki=snmdapeaki,snmdapeakp=snmdapeakp,
         snp=snp,srampai=srampai,srampap=srampap,srgabai=srgabai,srgabap=srgabap,srnmdai=srnmdai,srnmdap=srnmdap,t=t,taudampa=taudampa,
...
plt.savefig('raw_rate.svg')

#plt.figure()
#plt.plot(np.linspace(0,10000),i_rate*dt)
#plt.xlabel('time [ms]')
#plt.ylabel('network rate [spikes/s]')
#plt.savefig('binned_rate.svg')
...
plt.ylabel('power')
plt.savefig('network_rate_fft.svg')
#plt.semilogx(f,P1)
